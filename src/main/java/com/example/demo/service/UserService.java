...
public class UserService {
  ...
  private void validateEmail(UserInfoRequest request) {
    if (!EmailValidator.getInstance().isValid(request.getEmail())) {
      throw new CustomExсeption("Invalid email format", HttpStatus.BAD_REQUEST);
    }
  }
  ...
  public User getUserFromDb(Long id) {
    return userRepository
        .findById(id)
        .orElseThrow(() -> new CustomExсeption("User not found", HttpStatus.NOT_FOUND));
  }
  ...
  public UserInfoResponse updateUser(Long id, UserInfoRequest request) {
    ...
    user.setFirstName(
        request.getFirstName() == null ? user.getFirstName() : request.getFirstName());
    user.setLastName(request.getLastName() == null ? user.getLastName() : request.getLastName());
    user.setMiddleName(
        request.getMiddleName() == null ? user.getMiddleName() : request.getMiddleName());
  ...
  public Page<UserInfoResponse> getAllUsers(
    ...
    if (filter == null) {
      ...
      allUsers =
          userRepository.findAllByStatusNotFiltered(
              pageRequest, UserStatus.DELETED, filter.toLowerCase());
    }
